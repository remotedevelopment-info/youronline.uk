<script>
     ...
     * @prop {string} name - Name of the icon to render
     * @prop {string} [size="24"] - Size of the icon in pixels
     * @prop {string} [color="currentColor"] - Color of the icon
     * @prop {string} [class=""] - Additional CSS classes
     * @prop {string} [title=""] - Accessible title for the icon (for screen readers)
     */
    let { name, size = "24", color = "currentColor", class: className = "", title = "" } = $props();

    // Map of icon SVG paths
    const iconPaths = {
        // Navigation
        ...
        // Default icon (question mark) for when the requested icon doesn't exist
    ...
    // Use the requested icon path or default if it doesn't exist
    const iconPath = iconPaths[name] || iconPaths.default;
    
    // Generate a unique ID for the title element (for accessibility)
    const titleId = `icon-${Math.random().toString(36).substr(2, 9)}`;
</script>
